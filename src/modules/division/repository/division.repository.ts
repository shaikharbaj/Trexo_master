/**
 * @fileoverview
 * Division repository file to handle all division table operations
 *
 * @version
 * API version 1.0.
 *
...
export class DivisionRepository {
  ...
  async findOneWithoutDelete(select: any, where: any = {}) {
    return this.prismaService.division.findFirst({
      select: select,
      where: where,
    });
  }
  ...
  async create(payload: any) {
    return this.prismaService.division.create({ data: payload });
  }
  ...
  upsert(create: any, update: any, where: any) {
    return this.prismaService.division.upsert({
      create: create,
      update: update,
      where: where,
    });
