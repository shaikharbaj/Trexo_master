/**
 * @fileoverview
 * Contact us reply repository file to handle all contact us reply table operations
 *
 * @version
 * API version 1.0.
 *
...
@Injectable()
export class ContactUsReplyRepository {
    ...
    async findOneWithoutDelete(select: any, where: any = {}) {
        return this.prismaService.contactUsReply.findFirst({
            select: select,
            where: where,
        });
    }
    ...
    async create(payload: any) {
        return this.prismaService.contactUsReply.create({ data: payload });
    }
    ...
    async findMany(select: any, where: any = {}) {
        return this.prismaService.contactUsReply.findMany({
            select: select,
            where: {
                is_deleted: false,
                ...where,
    ...
    async findManyWithPaginate(page: number, select: any, where: any = {}) {
        ...
        return paginate(
            this.prismaService.contactUsReply,
            {
                select: select,
                where: {
                    is_deleted: false,
